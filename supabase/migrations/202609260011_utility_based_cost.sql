...
-- Cost is based on useful performance per published session. Games and kills
-- affect rates inside a session; they are not an ever-growing price base.
...
with player_rows as (
 ...
 select greatest(0,least(1000,round(1000*(
   .70*u.performance+.20*greatest(0,least(100,rating))/100+.10*greatest(0,least(100,reputation))/100
 )*(1-exp(-u.weighted_sessions/3.5)))))::integer cost
 from utility u cross join profile_values
 where p_target_type='player' and u.target_id=p_target_id
), organization_cost as (
 select coalesce(sum(public.u2_cost_for_target('player',tm.user_id,'main')),0)::integer cost
 from public.team_members tm
 where p_mode='main' and p_target_type='team' and tm.team_id=p_target_id
)
...
with history as (
 ...
 select tm.team_id target_id,coalesce(sum(public.u2_cost_for_target('player',tm.user_id,'main')),0)::integer cost
 from public.team_members tm where p_target_type='team' group by tm.team_id
), entities as (
