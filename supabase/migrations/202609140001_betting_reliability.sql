-- Make dynamic bet confirmation idempotent and trace every accepted bet back
-- to the quote that produced it. The own-organization restriction is kept.

begin;

alter table public.site_bets
  add column if not exists quote_id uuid references public.betting_quotes(id) on delete set null;

create unique index if not exists site_bets_quote_unique
  on public.site_bets(quote_id)
...
begin
  ...
  values(p_user_id, market_id, q.id, p_stake, q.offered_odds, payout)
