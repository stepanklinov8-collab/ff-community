...
-- A published session has one winner only: the single overall first place.
-- Sessions with several active groups are qualification stages and have no winner.
...
returns boolean language sql stable security definer set search_path=public as $$
 select public.u2_session_has_single_group(p_session) and exists(
  ...
  cross join lateral jsonb_array_elements(coalesce(cp.published->'standings','[]'::jsonb)) standing
  where cp.session_id=p_session
    and cp.first_published_at is not null
    and standing->>'place'='1'
    and (
      (p_team_id is not null and standing->>'teamId'=p_team_id::text)
      or (p_user_id is not null and standing->>'userId'=p_user_id::text)
    )
 )
$$;
...
select 'player'::text target_type,f.user_id target_id,f.game_id::text||':'||f.user_id::text id,f.mode,e.id event_id,s.id session_id,null::uuid clan_war_id,e.title,s.start_time occurred_at,
 f.nickname name_snapshot,f.kills,1::integer games,f.place,case when public.u2_session_winner(f.session_id,f.team_id,f.user_id) and f.win_order=1 then 1 else 0 end wins,f.deaths,f.assists,'current'::text source
...
union all
select 'team',f.team_id,f.game_id::text||':'||f.team_id::text,f.mode,e.id,s.id,null::uuid,e.title,s.start_time,r.name_snapshot,f.kills,1,f.place,case when public.u2_session_winner(f.session_id,f.team_id,null) and f.win_order=1 then 1 else 0 end,null::integer,null::integer,'current'
...
union all
select 'team',h.organization_id,h.id::text,h.mode,h.event_id,h.session_id,h.clan_war_id,h.event_title,h.occurred_at,h.organization_name,h.kills,null::integer,h.place,case when h.place is not null and public.u2_session_winner(h.session_id,h.organization_id,null) then 1 else 0 end,null::integer,null::integer,'legacy'
...
begin
 ...
 coalesce(sum(x.weight*case when public.u2_session_winner(x.session_id,x.team_id,x.user_id) and x.win_order=1 then 1 else 0 end),0),count(*),count(distinct x.session_id) filter(where public.u2_session_winner(x.session_id,x.team_id,x.user_id)),coalesce(sum(x.kills),0)
 ...
 select count(*),count(*) filter(where public.u2_session_winner(session_id,team_id,user_id)),coalesce(sum(kills),0) into n,wins_count,kills_count from public.competition_player_facts where user_id=p_user_id and mode in('tournament','training');
...
begin
 ...
 select count(distinct session_id) filter(where played and public.u2_session_winner(session_id,team_id,null)),count(*) filter(where played),coalesce(sum(kills),0) into wins_count,n,kills_count
