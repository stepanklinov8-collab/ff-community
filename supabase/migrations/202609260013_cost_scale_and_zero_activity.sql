...
-- Store prices in rubles with hundredth precision. There is no artificial
-- upper ceiling, but entities without a played session remain at 0.00.
drop function if exists public.u2_cost_for_target(text,uuid,text);
...
with player_rows as (
 ...
 )*(1-exp(-u.weighted_sessions/3.5))*(1+.25*ln(1+u.weighted_sessions))/100,2))::numeric cost
 ...
 select greatest(0,round(
   ...
   +1500*(.50+q.quality)*(.60+.40*q.confidence)*(1+.20*ln(1+q.weighted_sessions)))/100,2
 ))::numeric cost
 from (
 ...
 where p_mode='main' and p_target_type='team' and coalesce(s.weighted_sessions,0)>0
)
